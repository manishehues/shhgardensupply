...
class WECMF_General_Template{
	...
	public function __construct() {
		...
		add_action('wp_ajax_thwecmf_hide_banner', array($this, 'hide_premium_banner'));
    ...
    public function preview_template(){
		...
		if( WECMF_Email_Customizer_Utils::wecm_valid() && WECMF_Email_Customizer_Utils::is_user_capable() ){
			$template_name = WECMF_Email_Customizer_Utils::prepare_template_name( sanitize_text_field( $_POST['template_name'] ) );
			$content = isset( $_POST['content'] ) ? wp_kses_post( stripslashes( $_POST['content'] ) ) : '';
			$css = isset($_POST['css']) ? sanitize_textarea_field( stripslashes($_POST['css']) ) : '';
			WECMF_Email_Customizer_Utils::create_preview();
			$path_template = WECMF_Email_Customizer_Utils::preview_path($template_name);
			...
			$response = $this->save_template_file($content, $path_template);
		}
	...
	public function reset_preview(){
		...
		if( WECMF_Email_Customizer_Utils::is_user_capable() ){
			$deleted = WECMF_Email_Customizer_Utils::delete_preview();
		}
		if( $ajaxAction ){
    ...
    /**
     * Ajax callback function to save the template and test email
     *
	 * @return boolean action done or not
     */
	public function save_template_content(){
		$response = '';
		if( WECMF_Email_Customizer_Utils::is_valid_action() ){//Nonce, Capability
			...
			if( $action_index && $action_index == 'settings' && WECMF_Email_Customizer_Utils::wecm_valid() ){
				$render_data = isset($_POST['template_render_data']) ? wp_kses_post( trim( stripslashes( $_POST['template_render_data'] ) ) ) : false;
				$render_css = isset($_POST['template_render_css']) ? sanitize_textarea_field( stripslashes($_POST['template_render_css'] ) ) : '';
				$template_json = isset($_POST['template_json_tree']) ?  wp_kses_post( trim( stripslashes( $_POST['template_json_tree'] ) ) ) : ''; 
				if($render_data){
					$save_meta = false;						
	...
	public function hide_premium_banner(){
		$updated = false;
		if( WECMF_Email_Customizer_Utils::is_valid_banner_action() ){
			$timezone = new DateTimeZone( 'UTC' );
			$date = new DateTime("now", $timezone );
			$days_to_hide = sanitize_text_field( apply_filters( 'thwecmf_banner_expiry_time', '15 days') );
            $date = $date->modify("+{$days_to_hide}");
            $date = $date->format('Y-m-d H:i:s');
			$settings = WECMF_Email_Customizer_Utils::get_wecmf_misc_settings();
			$settings['banner_expiry'] = $date;
			$updated = WECMF_Email_Customizer_Utils::set_wecmf_misc_settings( $settings ); 
		}
	...
	public function save_template_files($template_name, $render_data, $render_css){
		...
		$path_template = esc_url( THWECMF_CUSTOM_T_PATH ).$template_name.'.php';
	...
	/**
     * Prepare template content and test email
     *
	 * @return mail sent or not
     */
	public function prepare_template_test_mail(){
		$message = $this->get_test_mail_message();
		$send_mail = $this->send_test_mail( $message );
		return $send_mail;
	}
	 ...
	 */	
	public function send_test_mail( $message ){
	...
	/**
     * Get the test mail message - template content
     *
	 * @return string $content template content
     */
	public function get_test_mail_message(){
		$order_id = isset( $_POST['id'] ) ? absint( $_POST['id'] ) : false;
		$email = isset( $_POST['email'] ) ? sanitize_text_field( $_POST['email'] ) : false;
		$template = isset( $_POST['name'] ) ? sanitize_text_field( $_POST['name'] ) : false;
		$content = $this->prepare_preview( $order_id, $email, $template, true );
		return $content;
	}
	...
	public function prepare_email_content_wrapper($content){
		...
		$wrapper .= '<tr>';
		$wrapper .= '<td align="center" class="thwec-template-wrapper-column" valign="top" style="'.esc_attr( $wrap_css ).'">';
		$wrapper .= $content;
		$wrapper .= '</td>';
		$wrapper .= '</tr>';
		$wrapper .= '</tr></table>';									
	...
	public function customer_data( $preview ){
		$class = $preview ? 'thwecmf-link' : '';
		$address = '<?php echo esc_html( $order->get_formatted_billing_full_name() ); ?><br><?php echo wc_make_phone_clickable( $order->get_billing_phone() ); ?><br><span class="'.$class.'" style="color:inherit;"><?php echo esc_html( $order->get_billing_email() ); ?></span>';
		return $address;
	}
	...
	public function billing_data( $preview ){
		...
		$address = '<?php echo wp_kses_post( $order->get_formatted_billing_address( esc_html__( "N/A", "woocommerce" ) ) ); ?>
				...
				<?php if ( $order->get_billing_email() ) : ?>
					<br><span class="'.$class.'" style="color:inherit;"><?php echo esc_html( $order->get_billing_email() ); ?></span>
				<?php endif; ?>';
		return $address;
	}
	...
	public function order_table_additional_td_css( $styles, $font_css ){
		...
		$fonts = WECMF_Email_Customizer_Utils::font_family_list();
	...
	public function prepare_preview( $order_id, $email_index, $template, $preview=false ){
		$content = '';
		$settings = WECMF_Email_Customizer_Utils::thwecmf_get_template_settings();
		if( WECMF_Email_Customizer_Utils::is_template( $template ) ){
			ob_start();
	        wc_get_template( 'emails/email-styles.php' );
	        $css = apply_filters( 'woocommerce_email_styles', ob_get_clean(), $this );
    		$css = $css.WECMF_Email_Customizer_Utils::get_thwecmf_styles();
    		$account_emails = array( 'WC_Email_Customer_New_Account', 'WC_Email_Customer_Reset_Password' );
    		$template = WECMF_Email_Customizer_Utils::get_template( $template, $preview );

	  		if( $template ){ 			
	  			$emails = WC_Emails::instance();
	  			$emails = $emails->get_emails();
				$email_class = isset( $emails[$email_index] ) ? $emails[$email_index] : false;	
				
				add_filter( 'woocommerce_email_recipient_' . $email_class->id, array( $this, 'no_recipient' ) );
				if( in_array( $email_index, $account_emails ) ){
					//Account related email
					$customer = WECMF_Email_Customizer_Utils::get_logged_in_user();
					if( $customer ){
						$template_type = 'html';
					...
					if( $email_index == 'WC_Email_Customer_Refunded_Order' ){
	...
	public function thwecmf_preview_email_type( $email_class ){
		$email_type = 'customer';
		if( in_array( $email_class->id, WECMF_Email_Customizer_Utils::THWECMF_EMAIL_INDEX ) ){
